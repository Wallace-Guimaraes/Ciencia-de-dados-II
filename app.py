...
def cor_risco(prob: float):
    if prob >= 0.65:
        return "#c0392b", "Alto", "risco-alto"
    elif prob >= 0.40:
        return "#d35400", "Médio", "risco-medio"
    else:
        return "#27ae60", "Baixo", "risco-baixo"
...
with st.form("form_simulador"):
    ...
    with c2:
        raca = st.selectbox("Raça / cor", options=RACAS, index=3)
    with c3:
        escolaridade = st.selectbox("Escolaridade", options=ESCOLARIDADES, index=4)

    c4, c5, c6 = st.columns(3)
    with c4:
        sit_conjugal = st.selectbox("Situação conjugal", options=SIT_CONJUGAL, index=1)
    with c5:
        relacao = st.selectbox("Relação com o agressor", options=RELACAO_AGRESSOR, index=0)
    with c6:
    ...
    with ce1:
        enc_delegacia      = st.checkbox("Delegacia")
    with ce2:
        enc_deam           = st.checkbox("DEAM")
    with ce3:
        enc_creas          = st.checkbox("CREAS")
    with ce4:
        enc_casa_mulher    = st.checkbox("Casa da Mulher")
    with ce5:
        enc_saude_mental   = st.checkbox("Saúde mental")
